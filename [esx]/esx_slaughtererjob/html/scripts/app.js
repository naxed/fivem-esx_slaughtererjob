	...
	let menus = {
		...
		cloakroom : {
		  ...
		  items: [
		  	...
		  	{label: 'Tenue abatteur', value: 'slaughterer_wear'}
		  ]
		},
	...
	window.onData = function(data){
		...
		if(data.enterPressed){
			...
			if(isShowingControls){
				...
				$.post('http://esx_slaughtererjob/select_control', JSON.stringify({
				...
				$.post('http://esx_slaughtererjob/select', JSON.stringify({
					menu : currentMenu,
					val  : currentVal,
					type : currentType,
					count: currentCount
		...
		if(data.backspacePressed){
			...
			if(isMenuOpen && currentMenu == 'alivechicken'){
				hideMenus();
				$('#ctl_return').hide();
				showControl('alivechicken');
			}

			if(isMenuOpen && currentMenu == 'slaughterhouse'){
				hideMenus();
				$('#ctl_return').hide();
				showControl('slaughterhouse');
			}

			if(isMenuOpen && currentMenu == 'packaging'){
				hideMenus();
				$('#ctl_return').hide();
				showControl('packaging');
			}
